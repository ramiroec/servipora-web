...
export default function RegistroProfesional() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(""); setExito("");
    try {
      await api.post("/usuarios/registro-profesional", form);
    ...
    <div style={{
      ...
      <div className="card" style={{
        ...
        <form onSubmit={handleSubmit} style={{ width: "100%", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <input name="cedula" placeholder="Cédula" value={form.cedula} onChange={handleChange} required style={{ padding: "12px", borderRadius: 10, border: "1px solid var(--stroke)", fontSize: 16 }} />
          <Select
          ...
          <textarea
            ...
            required
            style={{ padding: "12px", borderRadius: 10, border: "1px solid var(--stroke)", fontSize: 16, minHeight: 60 }}
          />
        ...
        <div style={{ marginTop: 18, color: "var(--muted)", fontSize: 14, textAlign: "center" }}>
          ¿Ya tienes cuenta? <span style={{ color: "var(--brand-2)", fontWeight: 600, cursor: "pointer" }} onClick={() => navigate("/login-profesional")}>Inicia sesión</span>
        </div>
      </div>
    </div>
  );
